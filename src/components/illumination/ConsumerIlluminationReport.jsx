...
import { Target, Users, Sparkles, GitCompare, CheckSquare, TrendingUp, Mic, RefreshCw } from 'lucide-react'
...
/** Flowing team PDF, logical pages from 15-slide deck content. */
export function ConsumerIlluminationReport({ sectionRefs }) {
  ...
  const segmentRefresh = slideById('segment-refresh')
  ...
  return (
    <div className="bg-slate-950">
      ...
      <ReportPage ref={setRef(2)}>
        ...
        <div className="mt-6 pt-4 border-t border-white/10">
          <ReportHeader
            title={segmentRefresh.title}
            titleHighlight={segmentRefresh.titleHighlight}
            subtitle={segmentRefresh.subtitle}
            icon={RefreshCw}
          />
          <IlluminationSlideBody slide={segmentRefresh} />
        </div>
        <div className="mt-6 pt-4 border-t border-white/10">
          <ReportHeader
            title={voice.title}
